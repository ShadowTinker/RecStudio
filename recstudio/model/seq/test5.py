...
class Test5(SASRec):
    ...
    def _init_model(self, train_data):
        ...
        self.graph_augmentation_model = data_augmentation.MyAugmentation(self.config['model'], train_data)
    ...
    def training_step(self, batch):
        ...
        cl_output2 = self.graph_augmentation_model(batch, self.item_encoder.weight[:-1])
        loss_value = self.loss_fn(batch[self.frating], **output['score']) \
           ...
           + self.config['model']['gcl_weight'] * cl_output2['cl_loss']
        return loss_value
