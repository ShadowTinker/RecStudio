...
class DuoRec(SASRec):
    ...
    def _init_model(self, train_data):
        ...
        self.augmentation_model = data_augmentation.DuoRecAugmentation(self.config['model'], train_data)
        self.augmentation_model2 = data_augmentation.CL4SRecAugmentation(self.config['model'], train_data)
        self.augmentation_model3 = data_augmentation.GSLAugmentation3(self.config['model'], train_data)
    ...
    def training_step(self, batch):
        ...
        output2 = self.forward(batch, False, return_query=True)
        cl_output = self.augmentation_model(batch, output['query'], output2['query'])
        cl_output2 = self.augmentation_model2(batch, self.query_encoder)
        if self.config['model']['graph_aug']:            
            cl_output3 = self.augmentation_model3(batch, self.item_encoder.weight[:-1])
        else:
            cl_output3 = {'cl_loss': 0}
        loss_value = self.loss_fn(batch[self.frating], **output['score']) + \
            ...
            self.config['model']['cl_weight'] * cl_output3['cl_loss']
        return loss_value
