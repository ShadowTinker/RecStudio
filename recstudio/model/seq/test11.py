...
class Test11(SASRec):
    def _init_model(self, train_data):
        ...
        self.augmentation_model2 = data_augmentation.ICLRecAugmentation(self.config['model'], train_data)
        # self.augmentation_model3 = data_augmentation.GSLAugmentation4(self.config['model'], train_data)
    ...
    def training_step(self, batch):
        ...
        cl_output2 = self.augmentation_model2(batch, output['query'], self.query_encoder)
        loss_value = self.loss_fn(batch[self.frating], **output['score']) \
           + self.config['model']['gcl_weight'] * cl_output['cl_loss'] \
           + self.config['model']['cl_weight'] * cl_output2['instance_cl_loss'] \
           + self.config['model']['intent_cl_weight'] * cl_output2['intent_cl_loss']
        return loss_value

    def training_epoch(self, nepoch):
        self.augmentation_model2.train_kmeans(self.query_encoder, self.trainloaders[1], \
