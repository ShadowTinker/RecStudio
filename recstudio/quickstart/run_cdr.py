...
def run_cdr(model: str, dataset: str, model_config: Dict=None, data_config: Dict=None, model_config_path: str=None, data_config_path: str=None, verbose=True, run_mode='light', **kwargs):
    ...
    meta_dataset, datasets = dataset_class(name=dataset, config=data_conf).build(**model_conf['data'])
    logger.info(f"{meta_dataset}")
    logger.info(f"\n{set_color('Model Config', 'green')}: \n\n" + color_dict_normal(model_conf, False))
    val_result = model.fit(meta_dataset, *datasets[:2], run_mode=run_mode)
